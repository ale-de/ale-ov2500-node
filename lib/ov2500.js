...
class OV {
        ...
        return Rx.Observable.fromPromise(new Promise((resolve, reject) => {
            request.post({
                ...
                try {
                    let result =this.result2JSON(body)[0];
                    let traps = result.response.trapData;
                    if (traps.length===0) {return reject("No notifications");}
                    if (traps.length > 1) traps.sort((a, b) => {
                        return a.instanceId - b.instanceId
                    });
                    resolve(traps);
                } catch (err) {return reject("JSON Error")}
            });
        })
        ).retry(3)
    }
    ...
    getMacAddresses(ip) {
        return new Promise((resolve, reject) => {
            let ipList="";
            ip.forEach((addr,idx)=>{ipList=ipList+addr+(idx!==ip.length-1?",":"")});
            request.post({
